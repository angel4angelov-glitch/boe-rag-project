...
- **Proves it works**: RAGAS metrics with proper statistics, giving **+0.148 retrieval precision** and **100% recall on questions that should be refused**.

---

## How it works

Solid arrows are fixed steps; **dashed arrows are decisions an LLM makes at runtime**, which is what makes it agentic rather than a fixed chain.
...
| 2 | **LLM query rewriting**: retry once on zero relevant chunks | Retrieval failure into empty context |
